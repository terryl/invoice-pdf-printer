...
### 🐛 问题修复
- 优化发票金额识别逻辑，支持解析“大写/小写”前缀格式的含税总金额
- 新增数电票二维码格式校验，跳过 XML/URL 等非标准格式，避免解析失败
- 调整金额读取优先级，优先采用从文本中提取的含税总额，确保数据准确
...
- 新增同名文件排除能力，避免重复文件进入待处理列表
- 在拖拽上传与文件选择两条入口统一接入按文件名去重逻辑
...
- 修复部分 PDF 因缺少中文字体无法显示缩略图的问题（本地化 CMap）
- 修复部分 PDF 无法嵌入导致合并结果为空的问题（自动回退到光栅化模式）
- 清理 index.html 中无用的脚本引用
